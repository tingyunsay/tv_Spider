...
import hashlib
from tv_Spider.Path_translate import Relative_to_Absolute,Get_Valid_Url,get_HeadUrl,Relative_to_Absolute2
from tv_Spider.Total_page_circulate import Total_page_circulate,Turn_True_Page
from tv_Spider.Get_Json_Content import Get_Json_Content
		...
		if 'All_Xpath' not in Final_Xpath.keys():
				...
				for key in my_Final_Xpath.keys():
						...
						try:
								...
								if map(lambda x:1 if x else 0, map(lambda x:response.xpath(x).extract() if x != "/" else "",my_Final_Xpath[key])) in [[0,0],[0]] and key != "site_name":		
										map(lambda x:l.add_value(key , ""),["just_one"])
								elif key == "site_name":
										map(lambda x:l.add_value(key , x),my_Final_Xpath[key])
								else:
										map(lambda x:l.add_xpath(key , x) if response.xpath(x).extract() != [] else "",my_Final_Xpath[key])
						except Exception,e:
								print Exception,":",e
				if Some_Info:
